...
function annotation_CRF_landmark_pc(strain,data,out_file,node_pot_type,atlasname,varargin)
...
load([data,'.mat'])
load(['D:\Dropbox (GaTech)\Whole-brain codes\CRF_Cell_ID-master\utils\data_neuron_relationship_' atlasname])
%load('C:\Users\hlee895\Desktop\CRF_Cell_ID-master\Run_SyntheticData\data_neuron_relationship')

%EntireAtlas= input("Enter 'y' to use the entire atlas. else enter 'n' - ")
EntireAtlas= 'n';
if EntireAtlas == 'n'
    %%% select subset of atlas based on data
    %keep_list= {'RIMR','RIML','RMDVL','RMDVR','RIGL','RIGR','SMDDL','SMDDR'}
    
% keep_list = {'AVAL', 'AVAR', 'AVEL', 'AVER', 'RIML', 'RIMR', 'RMDDL', 'RMDDR', 'RMDL', 'RMDR',...
%               'RMDVL', 'RMDVR', 'SMDDL', 'SMDDR', 'SMDVL', 'SMDVR', 'AVBL', 'AVBR', 'AVDL', 'AVDR',...
%               'AIBL', 'AIBR', 'URYVL', 'URYVR', 'AVG', 'RIGL', 'RIGR', 'RMGL','AVJR','AVJL',...
%               'AIAL', 'AIAR','URYDR', 'URYDL','RMEL','RMER','M1', 'RIAL',
%               'RIAR'}; % glr1 list (39)
         
         
% keep_list = {'ADAL','ADAR','AIML', 'AIMR', 'ALA', 'ASGL', 'ASGR', 'AVDL','AVDR', 'AVFL',...
%              'AVFR', 'AVG', 'CEPDL', 'CEPDR', 'CEPVL', 'CEPVR', 'OLLL', 'OLLR', 'URADL', 'URADR',...
%              'URAVL', 'URAVR', 'URYDL', 'URYDR', 'URYVL', 'URYVR',...
%              'RMEL','RMER','DB2','VA1','VB1','VB2','IL1VL','IL1VR'
%              'IL2VL','IL2VR'}; %acr5 list (36)

keep_list = {'ADAL', 'ADAR', 'AFDL', 'AFDR', 'AINL', 'AINR', 'ASGL', 'ASGR', 'ASHL', 'ASHR', 'AUAL', 'AUAR','AVJL','AVJR',...
             'AWBL', 'AWBR', 'BAGL', 'BAGR', 'FLPL', 'FLPR', 'GLRDL', 'GLRDR', 'GLRL', 'GLRR', 'GLRVL', 'GLRVR', 'RICL', 'RICR',...
             'RIGL', 'RIGR', 'RIML', 'RIMR', 'RIPL', 'RIPR', 'RMHL', 'RMHR', 'URBL', 'URBR'}; % inx4 list (38)
     
%Update the angular relationship
keep_index = [];
for i = 1:size(keep_list,2)
    if ~isempty(find(strcmp(keep_list{1,i},Neuron_head)))
        keep_index = [keep_index;find(strcmp(keep_list{1,i},Neuron_head))];
    else
        disp([keep_list{1,i} ' is not found in Neuron_head'])
    end    
end

keep_index= sort(keep_index,'ascend');

dummy_matrix = zeros(size(PA_matrix));
temp= [1:1:size(Neuron_head,1)];
temp(keep_index) = [];
remove_index= temp';
for i = 1:size(remove_index,1)
    dummy_matrix(remove_index(i,1),:) = 1;
    dummy_matrix(:,remove_index(i,1)) = 1;
end
rem_from_angle_vec_atlas = find(dummy_matrix == 1);
angle_vec_atlas(rem_from_angle_vec_atlas,:) = [];



DV_matrix = DV_matrix(keep_index,keep_index);
PA_matrix = PA_matrix(keep_index,keep_index);
LR_matrix = LR_matrix(keep_index,keep_index);
eigvec = eigvec(keep_index,keep_index);
eigval = eigval(keep_index,keep_index);
geo_dist = geo_dist(keep_index,keep_index);
ganglion = ganglion(keep_index,:);
LR_neurons = LR_neurons(keep_index,:);
LR_neurons_matrix = LR_neurons_matrix(keep_index,keep_index);
Neuron_head = Neuron_head(keep_index,:);
X_rot = X_rot(keep_index,:);
Y_rot = Y_rot(keep_index,:);
Z_rot = Z_rot(keep_index,:);
X_rot_norm = X_rot_norm(keep_index,:);

else
    keep_list= Neuron_head;
    keep_index=(1:size(Neuron_head,1))';
end %end for if entireatlas

% Randomly remove missing cells from atlas
%randomremove= input("Enter 'y' to randomly remove cells from the atlas (for multiple runs) - ")
randomremove= 'n';
if randomremove == 'y'
numLabelRem=size(keep_list,2)-size(mu_r,1);
if numLabelRem > 3
    numLabelRemove= numLabelRem -3; %-3 for glr-1
else
    
numLabelRemove=0;
end
elseif randomremove == 'n'
    numLabelRemove=0;
end

% delete landmark_names becasue it was mistakenly added during
% preprocessing (only for glr1 strain segmentation results that were
% created in 20210528)
%landmark_names=[];
...
remove_index = randperm(size(keep_index,1),numLabelRemove)';
...
dummy_matrix = zeros(size(PA_matrix) + numLabelRemove);
for i = 1:size(remove_index,1)
    dummy_matrix(remove_index(i,1),:) = 1;
    dummy_matrix(:,remove_index(i,1)) = 1;
end
...
%Only for glra data segmentation result created in 20210528
landmark_to_neuron_map=[];
...
% % %sequential labeling
% % whicheverismin= min(size(Neuron_head,1), size(mu_r,1));
% % cummulnodeBel=0;
% % disregardclamped=[];
% % Firstpriority=[];
% % Secondpriority=[];
% % Thirdpriority=[];
% % %Firstprioritylist= {'AVAL';'AVAR';'RMDDL';'RMDDR';'RMDVL';'RMDVR';'SMDDL';'SMDDR'};
% % %Secondprioritylist= {'RIML';'RIMR';'SMDVR';'SMDVL';'AVBL';'AVBR';'AVDL';'AVDR';'AVEL';'AVER';'RMDL';'RMDR';'RMEL';'RMER'};
% % Firstprioritylist= {'RMDVR';'SMDDR';'AVAL';'AVAR';'RMDDL';'RMDDR';'RMDVL';'SMDDL';'RIML';'RIMR';'SMDVR';'SMDVL'};
% % Secondprioritylist= {'AVBL';'AVBR';'AVDL';'AVDR';'AVEL';'AVER';'RMDL';'RMDR';'RMEL';'RMER'};
% % Thirdprioritylist= {};
% % for n= 1:size(Firstprioritylist,1) 
% % if ~isempty(find(contains(Neuron_head,Firstprioritylist(n))))
% %     Firstpriority= [Firstpriority find(contains(Neuron_head,Firstprioritylist(n)))];
% % end
% % end
% % for n= 1:size(Secondprioritylist,1) 
% % if ~isempty(find(contains(Neuron_head,Secondprioritylist(n))))
% %     Secondpriority= [Secondpriority find(contains(Neuron_head,Secondprioritylist(n)))];
% % end
% % end
% % for n= 1:size(Thirdprioritylist,1) 
% % if ~isempty(find(contains(Neuron_head,Thirdprioritylist(n))))
% %     Thirdpriority= [Thirdpriority find(contains(Neuron_head,Thirdprioritylist(n)))];
% % end
% % end
% % for j= 1: whicheverismin
% % [nodeBel,edgeBel,logZ] = UGM_Infer_Conditional(node_pot,edge_pot,edgeStruct,clamped,@UGM_Infer_LBP);
% % %cummulnodeBel= cummulnodeBel + nodeBel;
% % conserved_nodeBel = nodeBel; %node belief matrix to maintain marginal probabilities after clamping in subsequent steps
% % % optimal_decode = UGM_Decode_Conditional(node_pot,edge_pot,edgeStruct,clamped,@UGM_Decode_LBP);
% % for k= 1:size(disregardclamped,2) 
% % nodeBel(:,disregardclamped(k))=0;
% % end
% % for l= 1:size(Firstpriority,2) 
% % nodeBel(:,Firstpriority(l))=nodeBel(:,Firstpriority(l))*10;
% % end
% % for l= 1:size(Secondpriority,2) 
% % nodeBel(:,Secondpriority(l))=nodeBel(:,Secondpriority(l))*3;
% % end
% % for l= 1:size(Thirdpriority,2) 
% % nodeBel(:,Thirdpriority(l))=nodeBel(:,Thirdpriority(l))*1;
% % end
% % [sort_nodeBel,nodeBel_sort_index] = sort(nodeBel,2,'descend');
% % curr_labels = nodeBel_sort_index(:,1);
% % %sort_nodeBel(disregardclamped)= 0;
% % [~,max_nodeBel_loc]= max(sort_nodeBel(:,1));
% % disregardclamped= [disregardclamped nodeBel_sort_index(max_nodeBel_loc)];
% % clamped(max_nodeBel_loc,1) = nodeBel_sort_index(max_nodeBel_loc,1);
% % end
% % curr_labels= clamped;
% % curr_labels(find(curr_labels ==0))=-1;
% % 
...
% experiments(1).loc_sigma = loc_sigma; (not for uniform)
...
experiments(1).keep_list = keep_list;
...
% visualize_label_results_all(img_1,mu_r,Neuron_head,node_label)
end
