...
function preprocess_data(out_folder,data_name,img_1,img_green,img_1_marker,img_1_marker_name,varargin)
...
%     end
    ...
    thresh_param = 99.8;% 99.8;%99.8 for zhanglab images %threshhold for noisy segmentation%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %mu_noise(:,3)= mu_noise(:,3)*2/0.24;% scaling for sihoon's images
    % store segmentation information
    dummy = struct();
    ...
    axes_param = [1,3,2]; %input("Enter PCA coefficients for specifying AP, LR and DV axes e.g [1,2,3] or [1,3,2] - ");%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%change this
    PA_2 = coeff_2(:,axes_param(1,1))';
    PA_2 = PA_2/norm(PA_2);
    LR_2 = coeff_2(:,axes_param(1,2))';
    LR_2 = LR_2/norm(LR_2);
    DV_2 = coeff_2(:,axes_param(1,3))';
    DV_2 = DV_2/norm(DV_2);
    
    %Use the PA from noisy segmentation to find new LR and DV vectors in the
    %direction of LR from non-noisy segmentation
    fun = @(x)-(x(1)*LR_2(1,1) + x(2)*LR_2(1,2) + LR_2(1,3))^2/(x(1)^2 + x(2)^2 + 1^2);
    Aeq = PA(1,1:2);
    beq = -PA(1,3);
    % x0 = coeff(:,1);
    x0 = LR_2(1,1:2);
    LR_new = fmincon(fun,x0,[],[],Aeq,beq);   % LR axis (perperndicular to PA and in direction of LR neurons)
    LR_new = [LR_new,1];
    LR_new = LR_new/norm(LR_new);

    % DV axis (perperndicular to LR and PA axis)
    A = [PA(1,1:2);LR_new(1,1:2)];
    b = [-PA(1,3);-LR_new(1,3)];
    DV_new = inv(A'*A)*A'*b;           
    DV_new = [DV_new',1];
...
%     end
    ...
    %boundaryneeded = input("Enter 'y' if you want to specify a point along the AP axis below which cells are ignored - ");
    boundaryneeded= 'n'
    if boundaryneeded == 'y'
    X = mu_r*PA';
    Y = mu_r*LR';
    ...
    figure, imshow(max(mat2gray(img_1),[],3),[],'border','tight');
    hold
...
%     text(22*DV(1,1),22*DV(1,2),22*DV(1,3),'V','FontWeight','Bold','Color','k')
    ...
    save([out_folder,'\',data_name],'mu_r','mu_other_channels','labeled_img_r','labeled_img_other_channels','landmark_names','landmark_to_neuron_map','mu_c','img_1','varargin','cmap','ind_PCA','specify_PA','axes_neurons_to_neuron_map','axes_param','segNorm1','PA','DV','LR')
