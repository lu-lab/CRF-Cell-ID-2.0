function [x_thresh_new,y_thresh_new,z_thresh_new,index_thresh_new] = remove_close_neurons(x_thresh,y_thresh,z_thresh,index_thresh,img)
     
    dist_mat = squareform(pdist([x_thresh,y_thresh]));
    dist_mat = dist_mat + 1000*eye(size(dist_mat));
    close = dist_mat <= nthroot(150,2);%change how far they have to be in x and y dir
    dist_mat_z = squareform(pdist([z_thresh],'minkowski',1));
    dist_mat_z = dist_mat_z + 1000*eye(size(dist_mat_z));
    close_z = dist_mat_z <= 40;%change how many z-planes the cells have to be apart in order to be counted as separate
