...
%     [K,H,k1,k2] = curvature(img,find(ones(size(img))));
    ...
    sig_img_data = (cat(2,reshape(img_data(:,1),1,1,size(img_data,1)),reshape(img_data(:,2),1,1,size(img_data,1)),reshape(img_data(:,3),1,1,size(img_data,1))));
...
function [mu,sigma,comp_weights] = em_step(img,index_filt,img_data,I_norm,sig_img_data,iter,max_iter,mu,sigma,comp_weights,change_mu,cmap)
    ...
    while  abs(new_error - old_error) > 10^-3 && new_error < old_error && iter <= max_iter
        ...
        for k = 1:num_comp
            ...
            sig_mu_data = (repmat(mu(k,:),1,1,size(img_data,1)));
            sig = pagemtimes(permute(sig_img_data - sig_mu_data, [2 1 3]),sig_img_data - sig_mu_data);
            multiplied_ndarrays= times(sig,repmat(reshape(contribution(:,k),1,1,size(img_data,1)),size(sigma,1),size(sigma,2),1));
            sigma(:,:,k) = sum(multiplied_ndarrays,3)/sum(contribution(:,k));
        end
