function node_label = duplicate_labels(curr_labels,X,Y,Z,PA_matrix,LR_matrix,DV_matrix,geo_dist,geo_dist_r,thisimage_r,mu,Neuron_head,lambda_geo,clamped_neurons)%
%disp('duplicatelabelfunction started')
...
function score = consistency_score(i,i_label,node_label,X,Y,Z,PA_matrix,LR_matrix,DV_matrix,geo_dist,geo_dist_r,lambda_geo)
    ...
    score = (PA_consistency_score + LR_consistency_score + DV_consistency_score)/(4*max_consistency_score);
end