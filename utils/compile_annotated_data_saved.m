...
function compiled_data = compile_annotated_data(in_direc, whichworm_list)
    ...
    compiled_data = struct();
        ...
        %%% read marker files
        [X,Y,Z,marker_name,marker_index] = read_marker_files_annotation(in_direc{1,i});
        ...
        redImage=read_tif('one',[in_direc{1,i} '\worm' whichworm_list{1,i} 'GFP.tif']);
...
%         end
    ...
    %visualize    
    figure, imshow(max(mat2gray(redImage),[],3),[],'border','tight');
    hold
