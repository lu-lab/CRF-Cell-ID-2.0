...
load('D:\Dropbox (GaTech)\hyun jee share\CRF_Cell_ID-master\utils\data_neuron_relationship_annotation_updated.mat')

in_direc= cell(1, size(wormlist,1));
whichworm_list= cell(1, size(wormlist,1));
atlasname=[];
% for j= 1:28; %size(wormlist,1)
% whichworm= wormlist(j,:);
% in_direc{1,j}= ['D:\Dropbox (GaTech)\hyun jee share\CRF_Cell_ID-master\Images\glr1\worm' whichworm];
% whichworm_list{1,j}= whichworm;
% atlasname=[atlasname whichworm];
% end
for k= 1:size(wormlist,1)
whichworm= wormlist(k,:);
in_direc{1,k}= ['D:\Dropbox (GaTech)\hyun jee share\CRF_Cell_ID-master\Images\acr5\worm' whichworm];
whichworm_list{1,k}= whichworm;
atlasname=[atlasname whichworm];
end
out_direc= ['D:\Dropbox (GaTech)\hyun jee share\CRF_Cell_ID-master\utils\data_neuron_relationship_acr5_only10' ]; %_but_' exclude_worm ];
